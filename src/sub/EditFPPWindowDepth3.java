...
public class EditFPPWindowDepth3 extends Stage {
    public EditFPPWindowDepth3(Window window, String tree_name, LayerDataEx layerDataEx){
        ...
        SetAnchorPane(buttons_box, 200, 700);
            ...
            layerDataEx.getPolygons().get(0).setDotColor(colorPicker.getValue());
            close();
        });

        root.getChildren().addAll(color_select_box, buttons_box);

        System.out.println(layerDataEx.getPolygons().get(0).getDotColor());
